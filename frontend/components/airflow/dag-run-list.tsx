...
function StateBadge({ state }: { state: string }) {
  if (state === "success")
    return <span className="flex items-center gap-1 text-2xs font-medium text-[var(--dp-good-text)]"><CheckCircle2 className="h-3 w-3" />Success</span>
  if (state === "failed")
  ...
  if (state === "queued")
    return <span className="flex items-center gap-1 text-2xs font-medium text-[var(--dp-warn-text)]"><PlayCircle className="h-3 w-3" />Queued</span>
