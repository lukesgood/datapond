...
## 0. 이후 반영 (2026-09-16 추가)

아래 §1~§7은 **2026-09-07 시점의 판정**이며 그대로 둔다. 그 뒤 세 항목이 실제로 구현됐으므로,
이 문서를 근거로 로드맵을 판단할 때는 다음을 함께 읽어야 한다.

| 당시 판정 | 지금 | 근거 |
|---|---|---|
| 감사 — 미충족 | 성공한 도구 호출이 `tool_call_log`에 기록된다 | 마이그레이션 0008/0009, `app/tool_call_log.py`, 라이브 적재 확인 |
...
| 감사 | **미충족** (→ §0: 이후 반영됨) | 권한 거부·쓰기 허용만 기록. 성공한 search/rag/sql/query는 무엇을 물었는지 남지 않음 |
| 비용 통제 | **부분** (→ §0: 이후 강제 추가) | 호출자별 귀속·조회는 됨. 어떤 호출도 예산으로 거부되지 않음 |
...
| ISMS-P 증빙 형태(기간·행위자·자원·결과) | △ | §6.5 | NDJSON export에 기간·행위자·자원·결과가 한 행에 있음(`/api/audit/tool-calls/export`); 심사 대응 문서는 없음 |
...
| ISMS-P 증빙 | ✕ | 축 C | 축 C 참조(△) |

### 7.2 종합

| 축 | 보유율(항목 기준) | 판정 |
|---|---:|---|
| A 직접 호출 | 10 / 13 | 여정이 있고, MCP·OAuth가 남았다 |
| B 데이터 거버넌스 | 8 / 9 | 접근·감사 있음, 예산 강제 없음 (2026-09-16 추가됨 — §0) |
| C 심사 통과 | 4 / 6 | 리포트에 에이전트 호출 포함, 호출자 화면·ISMS-P 문서 없음 |
| D 온프렘 | 5 / 5 | 지원 프로필 있음(에어갭 acceptance는 과거 기록) |
| E 한국 세그먼트 | 2 / 5 | 마스킹은 있고 **UI·매핑이 없다** |

(2026-09-07 저녁 갱신: 세 계획 실행 후. 원래 값 A 5/13, B 6/9, C 2/6, D 2/5, E 2/5.)

세 조각이 이 축들의 빈칸을 채웠다.

1. **도구 호출 감사 로그** — B·C·E 세 축의 빈칸을 한 번에 채웠다(§6.4, §6.5).
2. **에이전트 온보딩 여정** — 스코프·만료가 있는 키 발급, 계정 쪽에서 컬렉션 부여, 컬렉션 페이지
   스니펫, `/connect`에 SQL 포함. A축의 빈칸 대부분이던 UI 작업이 끝났다.
3. **lean 온프렘 코어 프로필** — `values-sovereign-core.yaml`과 지원 티어 명시. D축은 코드가 아니라
   파일 하나와 문장 하나였다.

세 조각 모두 브랜치 `feat/positioning-gap-closure`에 반영됐다. 남은 항목은 §6.8의 순위 4~9에
정리되어 있다.
