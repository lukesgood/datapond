...
def _validation_enabled() -> bool:
    return os.getenv("AI_SQL_VALIDATE", "true").strip().lower() in ("1", "true", "yes", "on")


def validate_sql(sql: str):
    """(ok, error) from the query engine. Indirection keeps this patchable in tests."""
    from app.api.query_engine import validate_statement
    return validate_statement(sql)


_SQL_KEYWORDS = re.compile(r"\b(SELECT|WITH|INSERT|UPDATE|DELETE|CREATE|SHOW|DESCRIBE|EXPLAIN)\b", re.I)

# A statement keyword at the START OF A LINE is evidence of SQL; the same word in
# the middle of a sentence is not. SELECT, WITH, SHOW, UPDATE, CREATE and DESCRIBE
# are all ordinary English, so a bare \b match treats "...a table with columns
# like region..." as a query and hands the user a sentence to run. Observed live.
_STMT_START = re.compile(
    r"^[ \t]*(SELECT|WITH|INSERT|UPDATE|DELETE|MERGE|CREATE|DROP|ALTER|TRUNCATE"
    r"|SHOW|DESCRIBE|DESC|EXPLAIN|VALUES)\b",
    re.I | re.M,
)
# sqlglot falls back to Command for anything it cannot model — which is both a real
# `SHOW TABLES` and an unparsed English sentence. Accept Command only for the
# statement families that genuinely land there, and only without sentence punctuation.
_COMMAND_OK = re.compile(r"^[ \t]*(SHOW|DESCRIBE|DESC|EXPLAIN)\b[^.?!]*$", re.I | re.S)


def _is_sql_statement(candidate: str, dialect: str = "trino") -> bool:
    """Whether `candidate` parses as a real statement rather than an English sentence."""
    try:
        node = sqlglot.parse_one(candidate, read=dialect)
    except Exception:
        return False
    if node is None:
        return False
    if isinstance(node, exp.Command):
        return bool(_COMMAND_OK.match(candidate.strip()))
    return True


def _extract_statement(text: str, dialect: str = "trino") -> Optional[str]:
    """Return the SQL statement embedded in `text`, or None if there is none.

    Anchored to the start of a line so prose cannot match, then confirmed by an
    actual parse. Trailing commentary is common ("<sql>\n\nThis returns ..."), so
    fall back to shorter cuts before giving up.
    """
    text = (text or "").strip()
    m = _STMT_START.search(text)
    if not m:
        return None
    ...
    if ";" in tail:
        candidates.append(tail.split(";", 1)[0].strip())
    for cand in candidates:
        if cand and _is_sql_statement(cand, dialect):
            return cand
    return None
...
def _parse_response(raw: str) -> dict:
    ...
    # A broken JSON wrapper is the one case where cutting mid-line is correct: the SQL
    # really is in there, just wrapped (unescaped Trino "identifiers" break json.loads).
    # Without wrapper evidence, require the reply to actually parse as a statement —
    # otherwise a model that answers in prose gets its sentence returned as runnable SQL.
    if '"sql"' in body:
        salvaged = _salvage_sql(body)
        if salvaged and salvaged != body:
            return {"sql": salvaged, "explanation": "Generated by AI (non-JSON reply)."}
    stmt = _extract_statement(body)
    if stmt:
        return {"sql": stmt, "explanation": "Generated by AI (non-JSON reply)."}
...
class AskResponse(BaseModel):
    ...
    # None = not attempted (validation off, or no SQL to check); True/False = the
    # engine's own verdict from EXPLAIN (TYPE VALIDATE) against the live catalog.
    validated: Optional[bool] = None
    validation_error: Optional[str] = None
    # The model answered, but with a question rather than a query. The client must
    # show `explanation` and leave the editor alone — there is no SQL to paste.
...
async def generate_sql(req: AskRequest, user: dict = Depends(require_user)):
    ...
    # question used to come back dressed as SQL. Answer it ourselves, for free.
    if schema_ctx.strip() == "No tables found in the catalog.":
        return AskResponse(
            sql="",
            explanation="카탈로그에 테이블이 없습니다. Sources에서 데이터를 적재하거나 "
                        "Catalog에서 테이블을 등록한 뒤 다시 질문하세요.",
            has_ai=False,
    ...
    if cfg["litellm_url"]:
        ...
        try:
            ...
            # Only a transport/backend failure reaches the template fallback below.
            logger.warning(f"[ai_sql] gateway unavailable or no active backend: {e}")
        else:
            try:
                data = _parse_response(raw)
            except ValueError:
                # The model replied, just not with SQL — usually a clarifying question.
                # Hand its words back verbatim rather than inventing a statement.
                return AskResponse(
                    sql="",
                    explanation=(raw or "").strip()[:1000],
                    has_ai=True,
                    provider=provider,
                    pii_masked=pii_count,
                    needs_input=True,
            ...
            # ── Check it against the real catalog before the user sees it ─────
            # A generated query is plausible text until the engine resolves every
            ...
            if sql and _validation_enabled():
                ...
                if not ok:
                    repair = messages + [
                        ...
                        {"role": "user", "content":
                            f"That query failed validation against the catalog:\n{verr}\n"
                            "Return corrected JSON with the same two keys. Use only tables "
                            "and columns from the schema above."},
                    ]
                    try:
                        raw2 = await asyncio.to_thread(_call_litellm, system, repair)
                        ...
                        logger.warning(f"[ai_sql] validation repair round failed: {e}")
